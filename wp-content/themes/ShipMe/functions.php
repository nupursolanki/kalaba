...
if (!empty($cc) && shipme_using_permalinks())
    $raga_url_thing = $cc;
...
function shipme_send_email($recipients, $subject = '', $message = '') {
    ...
    if (empty($shipme_email_name_from))
        $shipme_email_name_from = "ShipMe Theme";
    if (empty($shipme_email_addr_from))
        $shipme_email_addr_from = "shipme@wordpress.org";
    ...
    if ($shipme_allow_html_emails != "yes")
        $html = false;
    else
        $html = true;
...
function shipme_get_priv_mess_page_url($subpage = '', $id = '', $addon = '') {
    ...
    if ($subpage == "delete-message") {
        if (!empty($_GET['rdr']))
            $rdr = urlencode($_GET['rdr']);
        else
            $rdr = urlencode(SH_curPageURL());
    }
    ...
    if (!function_exists('bcn_display')) {
        ...
        <div class="updated">
            <p><?php
                ...
                echo sprintf(__('In order to benefit of the full experience of our theme, we recommend installing <strong><a href="%s">the Breadcrumb Plugin</a></strong>.', 'shipme'), $ss);
                ?></p>
        </div>
        <?php
    }
...
function shipme_get_show_price($price, $cents = 0) {
...
function shipme_generate_thumb($img_ID, $width, $height, $cut = true) {
    ...
    if (empty($xx))
        $xx = shipme_wp_get_attachment_image(($img_ID - 1), array($width, $height));
    return $xx;
}
            ...
            if (count($r2) > 0)
                foreach ($r2 as $row2) { // = mysql_fetch_object($r2))
                    ...
                    if (isset($_POST['custom_field_value_' . $ids])) {
                        if ($_POST['custom_field_value_' . $ids] == $row2->valval)
...
function shipme_create_post_type() {
    ...
    register_post_type('job_ship', array(
        'labels' => array(
            'name' => __('Jobs', 'shipme'),
...
function shipme_get_images_cost_extra($pid) {
    ...
    if ($shipme_charge_fees_for_images == "yes") {
        ...
        if (empty($shipme_nr_of_free_images))
            $shipme_nr_of_free_images = 1;
...
function shipme_get_currency() {
    ...
    if (empty($c))
        return get_option('shipme_currency');
    return $c;
}
...
function shipme_get_weight_measure() {
    ...
    if (empty($shipme_get_weight_measure))
        return 'KG';
...
function shipme_get_dimensions_measure() {
    ...
    if (empty($shipme_get_dimensions_measure))
        return 'cm';
...
function shipme_get_post_new_error_thing($id) {
    ...
    if (is_array($class_errors)) {
        if (isset($class_errors[$id]))
            return $class_errors[$id];
    }
...
function shipme_get_post_new_error_thing_display($id) {
    ...
    if (is_array($MYerror)) {
        if (isset($MYerror[$id]))
            return '<p class="display-error-inside">' . $MYerror[$id] . '</p>';
    }
...
function shipme_post_new_with_pid_stuff_thg($pid, $step = 1, $fin = 'no') {
    ...
    if ($using_perm)
        return get_permalink(get_option('shipme_post_new_page_id')) . "?post_new_step=" . $step . "&" . ($fin != "no" ? 'finalize=1&' : '' ) . "jobid=" . $pid;
    else
        return get_bloginfo('siteurl') . "/?page_id=" . get_option('shipme_post_new_page_id') . "&" . ($fin != "no" ? 'finalize=1&' : '' ) . "post_new_step=" . $step . "&jobid=" . $pid;
}
...
function shipme_get_avatar($uid, $w = 25, $h = 25) {
    ...
    if (empty($av))
        ...
        return shipme_generate_thumb($av, $w, $h);
}
