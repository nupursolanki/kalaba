...
//-----------
	...
	$bid = shipme_get_bid_by_id($wp_query->query_vars['bid']);
	$pid = $wp_query->query_vars['pid'];
	$winner = get_post_meta($pid, 'winner', true);

	if(!empty($winner)) { 
		
		$shipme_enable_paypal_ad = get_option('shipme_enable_paypal_ad');
		if($shipme_enable_paypal_ad == "yes")
		{
		  wp_redirect(get_permalink(get_option('shipme_my_account_awaiting_completion_id')));	exit;	
		}
		
		//wp_redirect(get_permalink(get_option('shipme_my_account_page_id')));
		wp_redirect(shipme_get_payments_page_url2('escrow', $pid)); //get_permalink(get_option('shipme_my_account_page_id')));
...
//----------------------------------	
	...
	if(isset($_POST['yes']))
	{
		...
		$query = "update ".$wpdb->prefix."shipme_bids set date_choosen='$newtm', winner='1' where id='{$bid->id}'";
		$wpdb->query($query);
					...
					shipme_send_email_on_win_to_owner($pid, $uid);
						...
						global $wpdb;
						$s = "select distinct uid from ".$wpdb->prefix."shipme_bids where uid!='$uid' and pid='$pid'";
...
<div class="page_heading_me">
    ...
    <div class="main-pg-title">
    	<div class="mm_inn"><?php  printf(__("Choose Winner for your project: %s",'shipme'), $post_p->post_title); ?>
                     </div>
                    
 </div>


		<?php shipme_get_the_search_box() ?>            
                    
    </div>
</div>


<?php shipme_search_box_thing() ?>
